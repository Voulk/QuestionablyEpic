...
const getDamMult = (state, buffs, activeAtones, t, spellName, talents) => {
    let mult = SPECCONSTANTS.auraDamageBuff;
...
const getHealingMult = (state, t, spellName, talents) => {
    let mult = SPECCONSTANTS.auraHealingBuff ; 
...
export const runHeal = (state, spell, spellName, targetNum = 0) => {
    ...
    let healingVal = getSpellRaw(spell, currentStats, SPECCONSTANTS) * (1 - spell.expectedOverheal) * healingMult * targetMult;
...
export const runDamage = (state, spell, spellName, atonementApp, compile = true) => {
    ...
    const damageVal = getSpellRaw(spell, state.currentStats, SPECCONSTANTS) * damMultiplier;
...
export const runCastSequence = (sequence, stats, settings = {}, talents = {}, apl = []) => {
    ...
    const druidSpells = applyLoadoutEffects(deepCopyFunction(SPELLDB), settings, talents, state, stats);
    applyTalents(state, druidSpells, stats)

    const yserasGift = {...SPECCONSTANTS.yserasGift};
    yserasGift.flatHeal = getHealth(state.currentStats, talents) * 0.05;
    //addBuff(state, yserasGift, "Ysera's Gift");

    // Extra Settings
    //if (settings.masteryEfficiency) SPECCONSTANTS.masteryEfficiency = settings.masteryEfficiency;
