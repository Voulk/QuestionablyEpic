...
export const getShamanSpecEffect = (effectName, player, contentType) => {
    ...
    */
    ...
    if (player.getCovenant() === "necrolord"){
      ...
      if (pwavePerMinute > waveCasts) {
        triggerCasts += pwavePerMinute * avgRiptides * player.getFightLength(contentType) / 60;
      } else {
    ...
    // Each additional Healing Wave generated by Primordial Wave increases your Haste by 10% for 12 sec.
    const pwavePerMinute = (60 / 45) * 1.34; // TODO: Implement conduit properly, currently this is just 278
    const pwaveOver1 = (pwavePerMinute - 1) * 48 / 60; // This gets scaled down because you need to hold the pwave for 12s
    const expectedRiptides = 4.4; // TODO: Implement PTC bonus riptides? Or more implement this factor into PTC's value.
    const hasteEffectiveness = 5 / 6; // Based on stat weights
    const wastage = 0.7; // Lost healing, spells that haste has no effect on, overhealing, etc

    //bonusStats.hps = player.getHPS() * convertPPMToUptime(pwavePerMinute - 1 + pwaveOver1, 12) * 0.1 * expectedRiptides * hasteEffectiveness;
    bonusStats.hps = player.getHPS() * (pwavePerMinute * 12 * 0.1 * expectedRiptides) / 60 * wastage * hasteEffectiveness;
  ...
  } else if (effectName === "Skybreaker's Fiery Demise") {
    if (player.getCovenant() === "venthyr") {
      const targets = contentType == "Raid" ? 6 : 8;
      const chCooldown = 90 - (targets * 5 * (player.getStatPerc("Crit") + 0.15 - 1)); // TODO: Implement conduit properly, currently this is just 278
      ...
      const flameshockUptime = contentType == "Raid" ? 0.7 : 0.95; // TODO: Get better log data
      const flameshockCDR = 0.5 * player.getStatPerc("Haste") * (player.getStatPerc("Crit") - 1) * flameshockUptime; // CDR per second
      const skybreakerCapCrit = player.getStatPerc("Crit") + 0.5 > 2 ? 2 : player.getStatPerc("Crit") + 0.5;
      const skybreakerFSCDR = 0.5 * player.getStatPerc("Haste") * (skybreakerCapCrit - 1) * flameshockUptime; // CDR per second
      const fsDungeonAppliedCHCDR = contentType == "Raid" ? 0 : flameshockCDR * 18 * 5;
      const skybreakerDungeonAppliedCHCDR = contentType == "Raid" ? 0 : skybreakerFSCDR * 18 * 5;
      const flameshockTargets = contentType == "Raid" ? 1 : 2; // Can apply up to 3 normally, but typically will be less
      ...
      let skybreakerCD = chCooldown - skybreakerDungeonAppliedCHCDR;
      skybreakerCD = skybreakerCD / (1 + skybreakerFSCDR * flameshockTargets);
      ...
      const activeRiptideHeal = (((18 + 12 + 6) - 5.4 * 3) / 3) * 0.22 * player.getStatMultiplier("ALL") * shamanCorePassive;
      const rtLostHeal = contentType == "Raid" ? 3 / 20 * activeRiptideHeal : activeRiptideHeal; 
      const rtHealDifference = 5 * oneRiptide - rtLostHeal;

      const hpsDueToCDR = (rtHealDifference + oneChainHarvest) / skybreakerCD - (rtHealDifference + oneChainHarvest) / effectiveCD;

      bonusStats.hps = hpsDueToCDR; 
    } else { 
      bonusStats.hps = 0;
    }
  }
