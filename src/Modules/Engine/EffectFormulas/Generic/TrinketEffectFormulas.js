...
export function getTrinketEffect(effectName, player, contentType, itemLevel) {
  ...
  let activeTrinket = trinket_data.find(
    (trinket) => trinket.name === effectName
  );
  ...
  if (activeTrinket === undefined) {
    ...
    let base_heal = getProcessedValue(
      base_heal_effect.coefficient,
      base_heal_effect.table,
      itemLevel
    );
    let mana_heal =
      getProcessedValue(
        mana_heal_effect.coefficient,
        mana_heal_effect.table,
        itemLevel
      ) *
      (expected_mana_spend / 3240);

    bonus_stats.hps =
      (((base_heal + mana_heal) * base_heal_effect.efficiency) /
        base_heal_effect.cooldown) *
    ...
    bonus_stats.dps =
      (getProcessedValue(dam_effect.coefficient, dam_effect.table, itemLevel) /
        dam_effect.cooldown) *
      player.getStatMultiplier("CRITVERS");
    bonus_stats.intellect =
      (getProcessedValue(
        int_effect.coefficient,
        int_effect.table,
        itemLevel,
        int_effect.efficiency
      ) *
        int_effect.duration) /
      int_effect.cooldown;
    ...
    let effect = activeTrinket.effects[0];
    let playerBestSecondary = player.getHighestStatWeight(contentType, [
      "versatility",
    ]); // Exclude Vers since there isn't a Vers version.

    bonus_stats[playerBestSecondary] =
      getProcessedValue(effect.coefficient, effect.table, itemLevel) *
      convertPPMToUptime(effect.ppm, effect.duration);
    ...
    bonus_stats.hps =
      (getProcessedValue(
        heal_effect.coefficient,
        heal_effect.table,
        itemLevel,
        heal_effect.efficiency
      ) /
        heal_effect.cooldown) *
      player.getStatMultiplier("CRITVERS");
    bonus_stats.crit =
      (getProcessedValue(
        crit_effect.coefficient,
        crit_effect.table,
        itemLevel
      ) *
        crit_effect.duration *
        crit_effect.multiplier) /
      crit_effect.cooldown;
    ...
    bonus_stats.hps =
      (getProcessedValue(
        ...
        60) *
      effect.ppm *
      player.getStatMultiplier("NOMAST");
    ...
    bonus_stats.versatility =
      getProcessedValue(vers_effect.coefficient, vers_effect.table, itemLevel) *
      convertPPMToUptime(
        vers_effect.ppm * player.getStatPerc("Haste"),
        vers_effect.duration
      );
    ...
    // Health Potion Bonus
    bonus_stats.hps =
      ((10000 * 0.4) / player.getFightLength(contentType)) * 0.9; // 0.9 represents overhealing. We'll capture this better later.
    ...
    let hotHPS =
      (getProcessedValue(
        hotEffect.coefficient,
        hotEffect.table,
        itemLevel,
        hotEffect.efficiency
      ) *
    ...
    let absorbHPS =
      (getProcessedValue(
        absorbEffect.coefficient,
        absorbEffect.table,
        itemLevel,
        absorbEffect.efficiency
      ) *
        player.getStatPerc("Versatility")) /
      120;
    ...
    bonus_stats.hps =
      leechPercentage * expectedThroughput * effect.efficiency * uptime;
    ...
    if (trinket.name === "Darkmoon Deck: Repose") {
      console.log(
        trinket.name +
          " (i200): " +
          getEstimatedHPS(
            getTrinketEffect(trinket.name, player, contentType, 200),
            player,
            contentType
          )
      );
      ...
      let trinketEffect = getTrinketEffect(
        trinket.name,
        player,
        contentType,
        itemLevel
      ...
      console.log(
        trinket.name +
          ": " +
          JSON.stringify(trinketEffect) +
          ". Est HPS: " +
          getEstimatedHPS(trinketEffect, player, contentType)
      );
    }
  });
}
...
function getEstimatedHPS(bonus_stats, player, contentType) {
  ...
  for (const [key, value] of Object.entries(bonus_stats)) {
    if (["haste", "mastery", "crit", "versatility"].includes(key)) {
      estHPS +=
        ((value * player.getStatWeight(contentType, key)) /
          player.activeStats.intellect) *
        player.getHPS(contentType);
    } else if (key === "intellect") {
      estHPS +=
        (value / player.activeStats.intellect) * player.getHPS(contentType);
    } else if (key === "hps") {
      estHPS += value;
    }
  }
