...
export function getConduitFormula(effectID, player, contentType, itemLevel = 145) {
  ...
  switch (player.spec) {
    ...
    case (SPEC.HOLYPRIEST):
      return getPriestConduit(effectID, player, contentType, conduitRank);
    case (SPEC.RESTODRUID):
      return getDruidConduit(effectID, player, contentType, conduitRank);
    case (SPEC.RESTOSHAMAN):
      return getShamanConduit(effectID, player, contentType, conduitRank);
    case (SPEC.HOLYPALADIN):
      return getPaladinConduit(effectID, player, contentType, conduitRank);
    case (SPEC.MISTWEAVERMONK):
      return getMonkConduit(effectID, player, contentType, conduitRank);
    default:
      return {};
  }
}
