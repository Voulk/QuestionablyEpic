...
export const getMonkLegendary = (effectName, player, contentType) => {
    ...
    */
    if (name === "Ancient Teachings of the Monastery") {
        ...
        // Do Math
        const vivify = {cpm: player.getSpellCPM(IDVIVIFY, contentType), hps: player.getSpellHPS(IDVIVIFY, contentType), percentOnRemTargets: player.getSpecialQuery("percentVivifyOnRemTargets", contentType)};
        ...
        const renewingMist = {avgStacks: 2.9, // Can be closely modelled as VivifyHits / VivifyCasts - 1
                              oneSpread: player.getSingleCast(IDRENEWINGMIST, contentType) / 2} // ReMs spread at their current duration, which means we only get half of a ReM per spread on average.
