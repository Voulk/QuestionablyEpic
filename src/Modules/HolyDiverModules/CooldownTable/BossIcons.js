...
export default function bossIcons(props) {
  ...
  if (props === 2327) { source = MautIcon; alt = 'Maut' }
  if (props === 2328) { source = XaneshIcon; alt = 'Dark Inquisitor Xanesh' }
  if (props === 2329) { source = WrathionIcon; alt = 'Wrathion' }
  if (props === 2331) { source = RadenIcon; alt = 'Ra-den the Despoiled' }
  if (props === 2333) { source = HivemindIcon; alt = 'The Hivemind' }
  if (props === 2334) { source = SkitraIcon; alt = 'Prophet Skitra' }
  if (props === 2335) { source = ShadharIcon; alt = "Shad'har the Insatiable"}
  if (props === 2336) { source = VexionaIcon; alt = 'Vexiona'}
  if (props === 2337) { source = CarapaceIcon; alt = "Carapace of N'Zoth"}
  if (props === 2343) { source = DrestagathIcon; alt = "Drest'agath"}
  if (props === 2344) { source = NzothIcon; alt = "N'Zoth the Corruptor"}
  if (props === 2345) { source = IlgynothIcon; alt = "Il'gynoth, Corruption Reborn"}
  // Castle Nathria
  if (props === 2398) { source = ShriekwingIcon; alt = 'Shriekwing'}
  if (props === 2418) { source = HuntsmanAltimorIcon; alt = 'Huntsman Altimor' }
  if (props === 2402) { source = KaelthasSunstriderIcon; alt = "Sun King's Salvation" }
  if (props === 2405) { source = ArtificerIcon; alt = "Artificer Xy'mox" }
  if (props === 2383) { source = HungeringDestroyerIcon; alt = 'Hungering Destroyer' }
  if (props === 2406) { source = LadyInervaDarkveinIcon; alt = 'Lady Inerva Darkvein' }
  if (props === 2412) { source = CouncilIcon; alt = 'The Council of Blood' }
  if (props === 2399) { source = SludgefistIcon; alt = 'Sludgefist' }
  if (props === 2417) { source = StonebornGeneralsIcon; alt = 'Stone Legion Generals' }
  if (props === 2407) { source = SireDenathriusIcon; alt = 'Sire Denathrius' }
