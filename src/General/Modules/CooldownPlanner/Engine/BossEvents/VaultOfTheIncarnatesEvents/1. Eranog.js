...
export default function createEranogEvents(bossID, difficulty, damageTakenData, debuffs, starttime, enemyHealth, enemyCasts, buffData, friendlyHealth, enemyEnergy) {
  ...
  /*                                         Phase 2 Events                                         */
  /* ---------------------------------------------------------------------------------------------- */
  // console.log(enemyEnergyData.filter((filter) => filter.energy === 100));
  ...
  if (phase2One !== undefined && phase2Two !== undefined) {
    const timeDifference = Math.abs(phase2Two.time - phase2One.time);
    if (timeDifference <= 10000) {
      // phase2Three.time is within 25000 milliseconds of phase2Two.time
    } else {
      // phase2Three.time is not within 25000 milliseconds of phase2Two.time
      events.push({
  ...
  if (phase2Two !== undefined && phase2Three !== undefined) {
    const timeDifference = Math.abs(phase2Three.time - phase2Two.time);
    if (timeDifference <= 10000) {
      // phase2Three.time is within 25000 milliseconds of phase2Two.time
    } else {
      // phase2Three.time is not within 25000 milliseconds of phase2Two.time
