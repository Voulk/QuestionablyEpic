...
export async function importCooldownPlannerCastsLogData(starttime, endtime, reportid, healerID) {
  const APICast = "https://www.warcraftlogs.com:443/v1/report/events/casts/";
  const START = "?start=";
  const END = "&end=";
  const HOSTILITY = "&hostility=0";
...
// this can probably be merged with the normal damagelog function
export async function importDebuffDataFiltered(starttime, endtime, reportid, bossID) {
...
// this can probably be merged with the normal damagelog function
export async function importEnemyHealth(starttime, endtime, reportid) {
  const APIdamagetaken = "https://www.warcraftlogs.com:443/v1/report/tables/resources/";
  const API2 = "&api_key=92fc5d4ae86447df22a8c0917c1404dc";
  const START = "?start=";
  const END = "&end=";
  ...
  await axios
    .get(APIdamagetaken + reportid + START + starttime + END + endtime + HOSTILITY + resourceType + sourceClass + API2)
    .then((result) => {
      health = result.data;
