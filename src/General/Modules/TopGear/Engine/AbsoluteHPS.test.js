...
describe("buildDifferential", () => {
  const makeSet = (hardScore, setHPS) => ({
    hardScore,
    setHPS,
    itemList: [],
    enchantBreakdown: { Gems: [] },
  });

  test("carries absolute HPS and the gap against the best set", () => {
    const prime = makeSet(100000, 450000);
    const alternative = makeSet(99000, 445500);

    const differential = buildDifferential(alternative, prime, null, "Raid");
