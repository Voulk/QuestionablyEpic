...
export function durationmaker(ability, originalTimestamp, abilityname, endtime) {
  let duration = healerCooldownsDetailed.filter(obj => {
...
export async function importHealerLogData(starttime, endtime, reportid) {
  ...
  await axios.get(APIHEALING + reportid + START + starttime + END + endtime + apiMonk + API2)
    .then(result => {
  ...
  await axios.get(APIHEALING + reportid + START + starttime + END + endtime + apiPaladin + API2)
    .then(result => {
  ...
  await axios.get(APIHEALING + reportid + START + starttime + END + endtime + apiDruid + API2)
    .then(result => {
      healers = healers.concat(Object.keys(result.data.entries)
        .filter(key => (
  ...
  await axios.get(APIHEALING + reportid + START + starttime + END + endtime + apiPriest + API2)
    .then(result => {
      healers = healers.concat(Object.keys(result.data.entries)
        .filter(key => (
  ...
  await axios.get(APIHEALING + reportid + START + starttime + END + endtime + apiShaman + API2)
    .then(result => {
      healers = healers.concat(Object.keys(result.data.entries)
        .filter(key => (
...
export async function importDamageLogData(starttime, endtime, reportid) {
  ...
  await axios.get(APIdamagetaken + reportid + START + starttime + END + endtime + HOSTILITY + API2)
    .then(result => {
  ...
  do {
    await axios.get(APIdamagetaken + reportid + START + nextpage + END + endtime + HOSTILITY + API2)
      .then(result => {
...
export async function importCastsLogData(starttime, endtime, reportid, healerID) {
  ...
  await axios.get(APICast + reportid + START + starttime + END + endtime + HOSTILITY + API2)
    .then(result => {
  ...
  let i = 0
  do {
    await axios.get(APICast + reportid + START + nextpage + END + endtime + HOSTILITY + API2)
      .then(result => {
